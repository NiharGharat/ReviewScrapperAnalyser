...
import time
from impl.each_review import EachReview
...
class ReviewScrapper:
    ...
    logger_name = "ClassReviewScrapper"

    def __init__(self, log_file_name='file.log'):
        logging.basicConfig(format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S', level=logging.DEBUG)
        f_handler = logging.FileHandler(log_file_name)
        self.log = logging.getLogger(ReviewScrapper.logger_name)

    def process_review_block(self, review_block, pointer: int, list_of_reviews: list, number_of_reviews_required: int,
                             _number_of_reviews: int, all_scrape: bool) -> None:
        """
        ...
        :param number_of_reviews_required:
        :param _number_of_reviews:
        :param all_scrape:
        :return:
        """
        self.log.debug(">> processing_review_block()")
        self.log.debug("Processing review block %s", str(pointer))
        self.log.debug("Number of reviews in this block are ", str(len(list_of_reviews)))
        block = review_block[pointer].find_elements_by_css_selector("div[jscontroller^='fIQYlf']")
        self.log.debug("Block size is " + str(len(block)))
        for each_elem in block:
            # Name of reviewer
            reviewer_name_element = each_elem.find_elements_by_css_selector("div[class='TSUbDb']")
            self.check_component(reviewer_element=reviewer_name_element, element_name="reviewer_name_element",
                                 action="exception")
            ...
            self.check_component(reviewer_element=reviewer_age_element, element_name="reviewer_age_element",
                                 action="exception")
            # Rating
            reviewer_rating_element = each_elem.find_elements_by_css_selector("span[class='Fam1ne EBe2gf']")
            self.check_component(reviewer_element=reviewer_rating_element, element_name="reviewer_rating_element",
                                 action="exception")
            try:
                review_rating = reviewer_rating_element[0].get_attribute("aria-label")
            except NoSuchElementException as e:
                self.log.error("Star rating does not exists for the reviewer " + reviewer_name)
                raise e
                # review_rating = DEFAULT_RATING
            # No of reviews
            no_of_reviews_element = each_elem.find_elements_by_css_selector("span[class='A503be']")
            self.check_component(reviewer_element=no_of_reviews_element, element_name="no_of_reviews_element",
                                 action="exception")
            try:
                reviewer_no_of_reviews = no_of_reviews_element[0].text
            except Exception:
                self.log.warning("No no of review element, defaulting to 1")
            ...
            try:
                ...
                if more_text_block is None or len(more_text_block) < 1:
                    self.log.warning("No more element but no exception")
                    raise NoSuchElementException("No more element but no exception")
                self.log.debug("More exists for this reviewer " + reviewer_name)
                review_text = more_text_block[0].text
            except NoSuchElementException:
                self.log.debug("More does not exists for this reviewer " + reviewer_name)
            ...
            if (len(list_of_reviews) >= number_of_reviews_required) and not all_scrape:
                break
        self.log.debug("Block " + str(pointer) + " Done")
        self.log.debug("List size is " + str(len(list_of_reviews)))

    def check_component(self, reviewer_element, element_name: str, action: str) -> None:
        if len(reviewer_element) < 1:
            if action == _VALUE_RAISE_EXCEPTION:
                ...
                self.log.warning("Element {} was empty".format(element_name))
        elif len(reviewer_element) > 1:
            if action == _VALUE_RAISE_EXCEPTION:
                raise RuntimeError("Element {} has more elements".format(element_name))
            elif action == _VALUE_SUPPRESS_EXCEPTION:
                self.log.warning("Element {} has more elements".format(element_name))
        else:
            self.log.debug("Element {} is correct".format(element_name))

    def do_scrape(self, scrapper_input: ScrapperInput) -> List[MainReview]:
        # Setup done
        ...
        if scrapper_input.list_of_names is not None and len(scrapper_input.list_of_names_of_places) != 0:
            ...
            google_search_term = scrapper_input.single_name_of_place_to_scrape
            each_search_term = google_search_term.replace(" ", "+")
        ...
        driver.quit()
        self.log.info("All done, list of reviews were " + str(len(list_of_main_reviews)))
        return list_of_main_reviews

    def __do_each_scrape(self, scrapper_input: ScrapperInput, google_search_term: str, driver: webdriver) -> MainReview:
        complete_link = ReviewScrapper._google_link + google_search_term
        self.log.debug("Complete link created")
        ...
        try:
            ...
            reviews.click()
            self.log.info("Clicked on reviews")
            time.sleep(DEFAULT_INITIAL_SLEEP_SECONDS)
        except NoSuchElementException:
            self.log.error("No review element present for " + scrapper_input.single_name_of_place_to_scrape)
            return MainReview(name_of_apt=scrapper_input.single_name_of_place_to_scrape)
        ...
        self.log.debug("Number of reviews is " + str(_number_of_reviews))
        ...
        while True:
            ...
            self.process_review_block(review_block, _reviews_block_pointer, list_of_reviews, number_of_reviews_required,
                                      _number_of_reviews, all_scrape)
        ...
        self.log.info("List size is " + str(len(list_of_reviews)))
        ...
        if (len(list_of_reviews) == _number_of_reviews) or (len(list_of_reviews) == number_of_reviews_required):
            self.log.info("SUCCESS - Captured all the reviews")
        else:
            self.log.warning("WARN - Some issue, was not able to capture all reviews")

        self.log.info("Done, printing values")
